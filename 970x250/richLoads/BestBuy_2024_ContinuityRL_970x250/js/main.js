...
var Banner = {
	...
	init: function () {
		myFT.addEventListener("instantads", function () {
			...
			Images.add("images/arrow_next.svg", arrow_next);
			Images.add("images/arrow_prev.svg", arrow_prev);
			Images.add("images/FY2024_withoutGlowBG_970x250.png", logo);
			
			if (Banner.hasFrame4) {
				TweenMax.set(carousel, { width: "+=93" });
				TweenMax.set(arrow_next, { right: "+=93" });
			}
			...
			Images.add("images/image_970x250_logo_CircleR_v03.svg", intro_logo);
	...
	ready: function () {
		...
		mainTL.set("[id^=intro], [id^=CI], #carousel > div", {
			z: 0.01,
			rotation: 0.01,
			perspective: "1000px",
			force3D: true
	...
	playIntro: function () {
		...
		introTL.to(intro_logo, 0.5,{delay: 2,opacity:0},"-=0.5")
			.set(intro_logo, {scale: .68, x: -421,y:0, z: .01});
		
		introTL.addLabel("frame2");
		if (hasFrame3) {
			introTL.to([intro_logo, f2bg], .5, {opacity: 1}, 'frame2');
			introTL.to(CI_Frame2Copy, 1, {x: -10, opacity: 1, ease: Linear.easeNone}, "frame2")
				.to(CI_Frame2Copy, 1.7, {x: -130, ease: Linear.easeNone, delay: -.1})
				.to(CI_Frame2Copy, 1, {opacity: 0, x: -adWidth});
		}
		// return;
		introTL.to(intro_icon1, 1, { x: -162, ease: Linear.easeNone }, "-=1.0");
		// return;
		introTL.to(intro_icon1, 1.7, {x:-205, rotation: 0.01, ease: "none" })
	...
	playEndFrame: function () {
		...
		endFrameTL.to([carousel_items, ".product-info, .product-cta"], 0.35, {
			...
			onComplete: function () {
				...
				endFrameTL.set(".product.active", { x: 155 }, "fadein");
		...
		endFrameTL.set("#product_1 .product-cta", {x: 184, y: 22, scale: .95 },"fadein");   
		endFrameTL.to(legal, 0.35, { opacity: 1 }, "fadein");
		endFrameTL.to(arrow_prev, 0.35, { x: 150}, "fadein");
		endFrameTL.to(arrow_next, 0.35, { x: -254}, "fadein");
		endFrameTL.add(function () {
			...
			if (nextItem) {
				endFrameTL.to(nextItem, 0, { opacity: Banner.hasFrame4 ? 0 : 0.5 }, "shrink");
				endFrameTL.to(nextItem.querySelector(".product-image"), 0, { scale: 0.5 }, "shrink");
				endFrameTL.fromTo(nextItem, { x: "100%" }, { duration: 0.35, x: 386 }, "shrink+=0.35");
			}
			if (lastItem && lastItem !== nextItem && lastItem !== currItem) {
				endFrameTL.to(lastItem, 0, { opacity: 0.5 }, "shrink");
				endFrameTL.to(lastItem.querySelector(".product-image"), 0, { scale: 0.5 }, "shrink");
				endFrameTL.fromTo(lastItem, { x: "-100%" }, { duration: 0.35, x: -80 }, "shrink+=0.35");
			}
			// endFrameTL.set([f4_headline], { y: 60 });     
	...
	invokeArrowClick: function (e) {
		...
		if (Banner.carouselPlayer) {
			...
			Banner.carouselPlayer = null;
			TweenMax.to("#product_1 .product-cta", 0.35, { x:0,y: 0, scale: 1 });   
...
var Feed = {
	...
	success: function (items, url) {
		...
		for (var i = 0; i < productCount; i++) { 
			...
			var productImage = items[i].powerfeeds_image + "&w=200&h=200";
			...
			if (!isNaN(items[i].percentsavings) && Number(items[i].percentsavings) > 15) {
				dollarSign  = '<span style="color:#000;padding-right:1px;">$</span>';
				dollarSpSign  = '<span style="color: #000;padding-right:1px;">$</span>';
