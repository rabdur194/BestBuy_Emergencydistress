...
var Banner = {
	...
	init: function () {
		myFT.addEventListener("instantads", function () {
			...
			Images.add("images/image_300x50_logo_CircleR_v03.svg", intro_logo);
	...
	playIntro: function () {
		...
		introTL.to(intro_logo, 0.5, {delay: 2, opacity:0,},"-=0.5") 
			.set(intro_logo, {scale: .74, x: -112, y: 1, z: .01});
...
// return;
			...
			introTL.to([f2bg], 1.5 , {opacity: 1}, 'frame2'); 
			
		}
// return;
		introTL.to(intro_icon1, 0.5, { x: -125, ease: Linear.easeNone }, "-=0.8");
// return
		introTL.to(intro_icon1, 3, { transform: "translate3d(-150px, 2px, 1px)", ease: Linear.easeNone })
		 	.to(intro_icon1, 1, {opacity: 0, x: -adWidth});
		
		introTL.addLabel("endFrame", "=-.5");
// return;
		introTL.to(introFrame, 0.5, { opacity: 0 }, "endFrame");
		// introTL.to(hasFrame3 ? intro_frame3 : intro_frame2, 0.5, { scale: 10, rotation: 0.01 }, "endFrame");
