...
var Banner = {
	...
	init: function () {
		myFT.addEventListener("instantads", function () {
			...
			Images.add("images/FY2024_withoutGlowBG_160x600.png", logo);
			// Images.add("images/new_BB_Logo_160x600.png", logo);
			...
			Images.add("images/image_160x600_logo_CircleR_v03.svg", intro_logo);
	...
	playIntro: function () {
		...
		introTL.to(intro_logo,0.5,{delay: 2, opacity:0,},"-=0.5") 
			.set(intro_logo, {scale: .85, x: -28, y:-238, z: .01});
			// return;
		introTL.addLabel("frame2");
		if (hasFrame3) {
			introTL.to([intro_logo, f2bg], .5, {opacity: 1}, 'frame2')

			introTL.to(CI_Frame2Copy, 1, {x: -15, opacity: 1, ease: Linear.easeNone}, "frame2")
				.to(CI_Frame2Copy, 1.7, {x: -30, ease: Linear.easeNone, delay: -.1})
				.to(CI_Frame2Copy, 1, {opacity: 0, x: -adWidth});
			
			introTL.addLabel("frame3");
			}
		// return	
		introTL.to(hasFrame3 ? intro_frame3 : intro_frame2, 1, { x: 10, ease: Linear.easeNone }, "-=0.6");
		introTL.to(hasFrame3 ? intro_frame3 : intro_frame2, 1.7, {x:-5, rotation: 0.01, ease: "none" })
	...
	playEndFrame: function () {
		...
		endFrameTL.to([carousel_items, ".product-info, .product-cta"], 0.35, {
			...
			onComplete: function () {
				...
				endFrameTL.set(".product.active .product-image", { marginTop: 124 }, "fadein");
				endFrameTL.set(".product.active .product-info", { display: "none" }, "fadein");
				endFrameTL.set(".product.active .product-cta", { opacity: 1, marginTop: 12 }, "fadein");
			}
		});
		endFrameTL.add("fadein", ">");
		endFrameTL.to(carousel_items, 0.5, { opacity: 1, }, "fadein");
		endFrameTL.set("#product_1 .product-cta", { x:5,y: 170, scale: 1.07},"fadein"); 
		endFrameTL.to(legal, 0.35, { opacity: 1 }, "fadein");
		endFrameTL.to(arrow_prev, 0.35, { y: 18}, "fadein");
		endFrameTL.to(arrow_next, 0.35, { y: -166}, "fadein");
		endFrameTL.add(function () {
			...
			if (nextItem) {
				...
				endFrameTL.fromTo(nextItem, { y: "100%" }, { duration: 0.35, y: 131 }, "shrink+=0.35");
			}
			if (lastItem && lastItem !== nextItem && lastItem !== currItem) {
				...
				endFrameTL.fromTo(lastItem, { y: "-100%" }, { duration: 0.35, y: -123 }, "shrink+=0.35");
			}
			endFrameTL.to([f4_headline], 0.5, { opacity: 1, width:217, height:610, x:-28, y:25  });
		});
	...
	invokeArrowClick: function (e) {
		...
		if (Banner.carouselPlayer) {
			...
			if (currItem !== nextItem) {
				TweenMax.set(nextItem.querySelector(".product-cta"), { opacity: 1 });
	...
	success: function (items, url) {
		...
		for (var i = 0; i < productCount; i++) { 
			...
			if (!isNaN(items[i].percentsavings) && Number(items[i].percentsavings) > 15) {
				...
				dollarSpSign  = '<br/><span style="padding-right:1px;color: #000;">$</span>';
