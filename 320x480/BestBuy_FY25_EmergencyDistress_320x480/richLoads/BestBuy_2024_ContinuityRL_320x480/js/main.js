...
var Banner = {
	...
	init: function () {
		myFT.addEventListener("instantads", function () {
			...
			Banner.hasFrame4 = myFT.instantAds.Frame_4_Headline.match(/blank\.(png|gif)$/) == null;

			/* Build global elements */
			legal.innerHTML = myFT.instantAds.Legal_Text;
			Images.add(myFT.instantAds.Frame_4_Headline, f4_headline);
			Images.add("images/FY2024_withoutGlowBG_320x480.png", logo);
			// Images.add("images/BB_Logo_320x480.png", logo);
			Images.add("images/arrow_next.png", arrow_next);
			Images.add("images/arrow_prev.png", arrow_prev);

			Images.add("images/bg_320x480.png", testImg);
			...
			Images.add("intro_logo.png", intro_logo);
	...
	playIntro: function () {
		...
		introTL.to(intro_logo, 0.5, {delay: 2, opacity:0}, "-=0.5") 
			.set(intro_logo, {scale: .465, x: -99,y:-193, z: .01});   
		...
		introTL.set("#carousel",{opacity: 1});
		return introTL;
	},
	...
	playCarousel: function () {
		...
		// Banner.carouselPlayer.set("#carousel",{opacity: 1});
	...
	playEndFrame: function () {
		...
		endFrameTL.to([carousel_items, ".product-info, .product-cta"], 0.35, {
			...
			onComplete: function () {
				...
				endFrameTL.set(".product.active .product-image", { marginTop: 116 }, "fadein");
				endFrameTL.set(".product.active .product-info", { display: "none" }, "fadein");
				endFrameTL.set(".product.active .product-cta", { opacity: 1, marginTop: 19 }, "fadein");
			}
		});
		endFrameTL.add("fadein", ">");
		endFrameTL.to(carousel_items, 0.5, { opacity: 1, }, "fadein");
		endFrameTL.set("#product_1 .product-cta", { x:-4,y: 83, scale:0.97},"fadein"); 
		endFrameTL.to(legal, 0.35, { opacity: 1 }, "fadein");
		endFrameTL.to(arrow_prev, 0.35, { y: 45}, "fadein");
		endFrameTL.to(arrow_next, 0.35, { y: -80}, "fadein");
		endFrameTL.add(function () {
			...
			if (nextItem) {
				...
				endFrameTL.fromTo(nextItem, { y: "100%" }, { duration: 0.35, y: 200 }, "shrink+=0.35");
			}
			if (lastItem && lastItem !== nextItem && lastItem !== currItem) {
				...
				endFrameTL.fromTo(lastItem, { y: "-100%" }, { duration: 0.35, y: -100 }, "shrink+=0.35");
			}
